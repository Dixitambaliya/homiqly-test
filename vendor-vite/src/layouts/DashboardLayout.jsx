...
import { 
  FiHome, FiCalendar, FiShoppingBag, FiCalendarCheck, 
  FiBox, FiCreditCard, FiStar 
} from 'react-icons/fi';
...
const DashboardLayout = () => {
  ...
  const menuItems = [
    ...
    { path: '/calendar', name: 'Calendar', icon: <FiCalendar className="w-5 h-5" /> },
    { path: '/profile', name: 'Profile', icon: <FiUser className="w-5 h-5" /> },
    { path: '/services', name: 'My Services', icon: <FiShoppingBag className="w-5 h-5" /> },
    { path: '/bookings', name: 'Bookings', icon: <FiCalendarCheck className="w-5 h-5" /> },
    { path: '/supply-kits', name: 'Supply Kits', icon: <FiBox className="w-5 h-5" /> },
    { path: '/payments', name: 'Payments', icon: <FiCreditCard className="w-5 h-5" /> },
    { path: '/ratings', name: 'Ratings', icon: <FiStar className="w-5 h-5" /> },
  ];

  const getPageTitle = () => {
    const currentPath = location.pathname.substring(1);
    const menuItem = menuItems.find(item => item.path.substring(1) === currentPath);
          ...
          <div className="flex items-center justify-between px-6 py-4">
            ...
            <div className="flex items-center">
              <div className="flex items-center text-gray-600">
                <span className="mr-2">{currentUser?.name || 'Vendor User'}</span>
