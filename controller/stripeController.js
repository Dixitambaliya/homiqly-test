...
exports.stripeWebhook = asyncHandler(async (req, res) => {
  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    try {
      // ✅ Step 1: Get booking_id from payments table
      const [paymentRows] = await db.query(
        `SELECT booking_id, user_id, amount, currency FROM payments WHERE payment_intent_id = ? LIMIT 1`,
        [paymentIntentId]
      );

      if (!paymentRows || paymentRows.length === 0) {
        console.warn("⚠️ No payment found for PaymentIntent:", paymentIntentId);
        return;
      }

      const payment = paymentRows[0];
      const { booking_id, user_id, amount, currency } = payment;

      // ✅ Step 2: Mark payment completed
      await db.query(
        `UPDATE payments SET status = 'completed' WHERE payment_intent_id = ?`,
        [paymentIntentId]
      );

      // ✅ Step 3: Update service_booking with payment_intent_id and mark payment_status
      await db.query(
        `UPDATE service_booking SET payment_status = 'completed', payment_intent_id = ? WHERE booking_id = ?`,
        [paymentIntentId, booking_id]
      );

      // ✅ Step 4: Fetch booking + user + vendor details
      ...
      if (!userInfo || userInfo.length === 0) {
        console.warn("⚠️ No user info found for booking:", booking_id);
        return;
      }
      ...
      const packageHTML = packages.map(pkg => `
        <div style="border:1px solid #ccc; padding:10px; margin-bottom:10px;">
          ...
          <p>Price: ${pkg.totalPrice} ${currency}</p>
      ...
      const mailOptions = {
        ...
        html: `
          <h3>Hi ${user.name},</h3>
          <p>Your payment of <strong>${currency.toUpperCase()} ${amount}</strong> was successful and your booking is confirmed.</p>
      ...
      console.error("❌ Error during Stripe webhook:", err.message);
    }
  } else {
