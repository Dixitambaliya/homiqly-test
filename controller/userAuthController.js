...
const registerUser = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.query(userAuthQueries.userInsert, [firstname, lastname, email, phone]);
        ...
        res.status(200).json({ message: "Verification code sent to email.", user_id: result.insertId });
...
const setPassword = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const [result] = await db.query(userAuthQueries.userSetPassword, [hashedPassword, email]);

        if (result.affectedRows === 0) {
            return res.status(404).json({ error: "User not found or already has a password." });
        }
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!user.password || typeof user.password !== 'string') {
            return res.status(400).json({ error: "Password not set. Please reset your password." });
        }
...
module.exports = { registerUser, loginUser, verifyCode, setPassword, requestReset, verifyResetCode, resetPassword };
