...
export const VendorAuthProvider = ({ children }) => {
  ...
  const verifyResetCode = async (email, resetCode) => {
    try {
      setError(null);
      const response = await axios.post('/api/vendor/verifyresetcode', { email, resetCode });
      return { success: true, token: response.data.token };
    } catch (err) {
      setError(err.response?.data?.error || 'Invalid code');
      return { success: false, error: err.response?.data?.error || 'Invalid code' };
    }
  };
  ...
  const resetPassword = async (token, newPassword) => {
    try {
      setError(null);
      await axios.post('/api/vendor/resetpassword', 
        { newPassword },
