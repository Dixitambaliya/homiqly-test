...
import { FiCalendar, FiClock, FiUser, FiMapPin, FiCheckCircle, FiXCircle } from 'react-icons/fi';
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (selectedBookings.length > 0) {
          setSelectedBookings(selectedBookings.map(booking => 
            booking.booking_id === bookingId || booking.bookingId === bookingId
              ? { ...booking, bookingStatus: status }
              : booking
          ));
        ...
        <div className="flex items-center space-x-4">
          <div className="flex bg-gray-100 rounded-md overflow-hidden">
            <button 
              ...
              className={`px-3 py-1 text-sm ${viewMode === 'month' ? 'bg-primary-light text-white' : 'text-gray-600 hover:bg-gray-200'}`}
            >
              Month
            </button>
            <button 
              onClick={() => setViewMode('week')} 
              className={`px-3 py-1 text-sm ${viewMode === 'week' ? 'bg-primary-light text-white' : 'text-gray-600 hover:bg-gray-200'}`}
            >
              Week
            </button>
            <button 
              onClick={() => setViewMode('day')} 
              className={`px-3 py-1 text-sm ${viewMode === 'day' ? 'bg-primary-light text-white' : 'text-gray-600 hover:bg-gray-200'}`}
            >
              Day
            </button>
          </div>
              ...
              {dayBookings.slice(0, 3).map(booking => (
                <div 
                  ...
                  className={`text-xs p-1 rounded truncate ${
                    booking.bookingStatus === 0 ? 'bg-yellow-100 text-yellow-800' :
                    booking.bookingStatus === 1 ? 'bg-green-100 text-green-800' :
                    'bg-red-100 text-red-800'
                  }`}
                >
                  {booking.bookingTime.substring(0, 5)} - {booking.userName || 'Customer'}
                </div>
              ))}
        ...
        <div className="grid grid-cols-7 divide-x h-[500px] overflow-y-auto">
          {days.map(day => {
            ...
            return (
              <div 
                key={day.toISOString()} 
                className={`p-2 overflow-y-auto ${isToday ? 'bg-primary-50' : ''}`}
              >
                {dayBookings.length > 0 ? (
                  dayBookings.map(booking => (
                    <div 
