...
const Services = () => {
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      setFormData((prev) => ({
        ...
        packageMedia: file, // Assuming packageMedia is also the same file for simplicity
      }));
  ...
  const handlePackageMediaChange = (index, file) => {
    setFormData((prev) => {
      const updatedPackages = [...prev.packages];
      updatedPackages[index].packageMedia = file;
      return {
        ...prev,
        packages: updatedPackages,
      };
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !formData.packages.every(
        (pkg) => pkg.package_name && pkg.total_price && pkg.total_time
      )
    ) {
    ...
    try {
      ...
      formData.packages.forEach((pkg, idx) => {
        if (pkg.packageMedia) {
          formDataToSend.append(`packageMedia_${idx}`, pkg.packageMedia);
        }
      });
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          ...
                          <div>
                            ...
                            <input
                              type="file"
                              id={`packageMedia_${index}`}
                              accept="image/*"
                              className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100"
                              onChange={(e) =>
                                handlePackageMediaChange(
                                  index,
                                  e.target.files[0]
                                )
                              }
