...
import { FiCalendarCheck, FiClock, FiCheckCircle, FiDollarSign } from 'react-icons/fi';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const bookingsResponse = await axios.get('/api/booking/vendorbookedservices');
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white rounded-lg shadow p-6 flex items-center space-x-4">
          <div className="p-3 rounded-full bg-blue-100 text-blue-600">
            <FiCalendarCheck className="h-6 w-6" />
          </div>
          <div>
          ...
          {recentBookings.length > 0 ? (
            <div className="divide-y">
              {recentBookings.map((booking) => {
                const statusClass = 
                  booking.bookingStatus === 1 ? 'bg-green-100 text-green-800' : 
                  booking.bookingStatus === 2 ? 'bg-red-100 text-red-800' : 
                  'bg-yellow-100 text-yellow-800';
                
                const statusText = 
                  booking.bookingStatus === 1 ? 'Completed' : 
                  booking.bookingStatus === 2 ? 'Cancelled' : 
                  'Pending';
                
                return (
                  <div key={booking.bookingId || booking.booking_id} className="py-3 flex justify-between items-center">
                    <div>
                      <p className="font-medium">{booking.userName}</p>
                      <p className="text-sm text-gray-500">
                        {booking.serviceName} - {new Date(booking.bookingDate).toLocaleDateString()} {booking.bookingTime}
                      </p>
                    </div>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/vendor/calendar" className="text-sm text-primary-light hover:text-primary-dark font-medium">
            View Calendar
          </Link>
        </div>
        
        <div className="border rounded-lg p-4">
          <p className="text-center text-gray-500">Calendar component will be integrated here</p>
        </div>
      </div>
    </div>
  );
