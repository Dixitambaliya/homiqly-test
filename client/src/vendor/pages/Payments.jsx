...
const Payments = () => {
  ...
  const fetchPayments = async () => {
    try {
      setLoading(true);
      const response = await axios.get('/api/payment/vendor/history');
  ...
  const calculateStats = (paymentData) => {
    const totalEarnings = paymentData.reduce((sum, payment) => sum + payment.amount, 0);
    ...
    const completedAmount = paymentData
      ...
      .reduce((sum, payment) => sum + payment.net_amount, 0);
    const commissionPaid = paymentData.reduce((sum, payment) => sum + payment.commission_amount, 0);
  ...
  const exportToCSV = () => {
    // Create CSV content
    const headers = ['Payment ID', 'Service', 'Amount', 'Commission', 'Net Amount', 'Status', 'Date'];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div className="bg-white rounded-lg shadow p-4">
          ...
          <p className="text-2xl font-bold text-purple-600">{formatCurrency(stats.commissionPaid)}</p>
        </div>
      </div>
                ...
                {filteredPayments.map(payment => (
                  <tr key={payment.payment_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="text-xs text-gray-500">{payment.serviceCategory}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-gray-900">{formatCurrency(payment.amount)}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{formatCurrency(payment.commission_amount)}</div>
                      <div className="text-xs text-gray-500">{payment.commission_rate}%</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-medium text-green-600">{formatCurrency(payment.net_amount)}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
